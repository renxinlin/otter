 ...
 *
 * SelfMonitor
 *   GlobalMonitor          ----restartAlarmRecovery【获取channel.isPause状态重启】
 *      PipelineMonitor ---- arbitrateManageService
 *
 *
 *       delayStatRuleMonitor;                 ----AlarmController[restartAlarmRecovery 根据规则配置是否自动恢复判断是否恢复]
 *
 *       exceptionRuleMonitor;                 ----AlarmController[restartAlarmRecovery 根据规则配置是否自动恢复判断是否恢复]
 *
 *       pipelineTimeoutRuleMonitor;           ----AlarmController[restartAlarmRecovery 根据规则配置是否自动恢复判断是否恢复]
 *
 *       processTimeoutRuleMonitor;            ----AlarmController[restartAlarmRecovery 根据规则配置是否自动恢复判断是否恢复]
 *
 *       positionTimeoutRuleMonitor;           ----AlarmController[restartAlarmRecovery 根据规则配置是否自动恢复判断是否恢复]
