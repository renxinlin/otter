...
public class OtterController implements NodeTaskListener, OtterControllerMBean {
    ...
    public void stop() throws Throwable {
        ...
        try {
            ...
            arbitrateManageService.nodeEvent().destory(Long.valueOf(nid)); // todo 这里会导致manager调度nid上的channel
        } catch (Exception e) {
        ...
        try {
            nodeTaskService.stopNode(); // 通知manager停止当前node   todo 这里会导致manager调度nid上的channel
        } catch (Exception e) {
    ...
    public void startPipeline(NodeTask nodeTask) {
        ...
        // 处理具体的任务命令   SELECT, EXTRACT, TRANSFORM, LOAD;
    ...
    private void initNid() {
        ...
        // 这里主要检查manager的nid对应的ip:port和这个手动配置的nid对应的ip:port是否相等
        // TODO 吐槽下这个点，这个服务注册发现的能力太弱了
        checkNidVaild(nid);
        // zk上创建这个节点
        ...
        // session 失效后重建这个节点 todo  估计下文碰到watch的也需要重建
