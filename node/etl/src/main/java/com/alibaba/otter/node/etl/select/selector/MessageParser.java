...
public class MessageParser {
    ...
    private static final String compatibleMarkTable = "retl_client"; // 这个应该是otter 3.0的数据回环表 4.x没有了这张表 但做了兼容
     ...
     * renxl
     * 这是一条回环事务在mysql 上的格式
     * mysql-bin.000007	24762	Query   	1	24830	BEGIN
     * mysql-bin.000007	24830	Table_map	1	24886	table_id: 126 (retl.retl_mark)
     * mysql-bin.000007	24886	Update_rows	1	24948	table_id: 126 flags: STMT_END_F
     * mysql-bin.000007	24948	Table_map	1	25017	table_id: 122 (netty.app_info)
     * mysql-bin.000007	25017	Update_rows	1	25183	table_id: 122 flags: STMT_END_F
     * mysql-bin.000007	25183	Table_map	1	25239	table_id: 126 (retl.retl_mark)
     * mysql-bin.000007	25239	Update_rows	1	25301	table_id: 126 flags: STMT_END_F
     * mysql-bin.000007	25301	Xid	        1	25332	COMMIT * xid=452 *
     * <p>
     * <p>
     * 其中 Table_map会被canal过滤
     * <p>
     * 到达otter如下
     * <p>
     * * mysql-bin.000007	24762	Query   	1	24830	BEGIN
     ...
     * <p>
     * <p>
     * index [1]和index[3] 是otter的回环处理标志 tableid 126对应的表就是retl_mark
     */
            ...
            for (Entry entry : datas) {
                ...
                switch (entry.getEntryType()) {
                    ...
                    case ROWDATA:
                        ...
                        if (
                                (!isLoopback
                                        || (enableLoopbackRemedy && needLoopback)
                                )
                                        && !isMarkTable
                            ...
                            // 这里只是添加了业务的binlog  并且不能是回环数据 [假如是回环数据  只是处理需要进行回环补救算法的数据]
                            transactionDataBuffer.add(entry);
                        }
                        break;
                    case TRANSACTIONEND:
                        // 当一个事务结束 判断 如果不是回环数据 或者处理主站的冲突
                        if (!isLoopback || (enableLoopbackRemedy && needLoopback)) {
                            // 添加数据解析 【transactionDataBuffer 一个只有业务方的事务数据】
                            for (Entry bufferEntry : transactionDataBuffer) {
                                // canal 对象解析成otter对象 其中 EventData.cloumns里面存储了最新的一份所有字段值 【insert 的aftercolumns  delete 的before update 是after覆盖before】
     ...
     *
     *
     *  先比较channel_info 后比较channel_id
     *
     *  0  非回环数据  1回环数据 2 回环数据并且进行补救
     */
    private int checkLoopback(Pipeline pipeline, RowData rowData) { // rowData
        // 检查channel_info字段
        // 首先检查下after记录，从无变有的过程，一般出现在事务头
        Column infokColumn = getColumnIgnoreCase(rowData.getAfterColumnsList(), pipeline.getParameters()
                .getSystemMarkTableInfo());     // channel_info
    ...
    /**
     * 和checkLoopback 一样 针对的是3.x的otter版本  这里一般不会出现调用
