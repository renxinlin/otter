 ...
 *
 *
 * 启动canal instance 获取数据
 * 交给 select task处理获取的binlog
 * 
...
public class CanalEmbedSelector implements OtterSelector {
    ...
    public void start() {
        ...
        final boolean syncFull = pipeline.getParameters().getSyncMode().isRow()
                                 || pipeline.getParameters().isEnableRemedy(); // 是否启用冲突补救算法
        // 暂时使用skip load代替   是否跳过select时的执行异常
    ...
    public Message<EventData> selector() throws InterruptedException {
        ...
        // 拉取数据 拉取的不一定是一个完整的事务,二是batch的大小
        ...
        // otter  eventDatas可能没有到事务结尾  [打上冲突检测和延时反查处理  过滤回环标志binlog  事务头尾event]
